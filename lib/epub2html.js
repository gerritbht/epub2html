...
module.exports.convertMetadata = function convertMetadata(epubData)
{
	try {
		...
		console.log('Error converting metadata: ', e);
	}
	return {
		htmlNav: htmlNav,
		htmlMetas: htmlMetas,
...
function processNavMap(epubData) {
		...
		//If there is no navMap use the ePub3 equivalent
		if(typeof(epubData.raw.json.ncx) == 'undefined') {
			//var navpoints = epubData.raw.json.epub3NavHtml
			return (epubData.easy.epub3NavHtml);
		} else {
			var htmlNav = "<ul>\n";
		...
		function processNavPoint(np) {
			...
			if(typeof np.navLabel !== 'undefined') {				
				text = np.navLabel[0].text[0].toString('binary'); //toString('binary') to support special characters
			}
